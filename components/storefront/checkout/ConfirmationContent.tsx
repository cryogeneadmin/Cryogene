...
export function ConfirmationContent({
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-16">
      ...
      <p className="mono text-sm text-muted mb-12">Order {order.orderNumber}</p>
      ...
      <div className="bg-white border border-border p-6 mb-8">
        ...
        <div className="mt-4 pt-4 border-t border-border space-y-1 text-sm">
          <div className="flex justify-between"><span className="text-muted">Subtotal</span><span>{formatPriceFromPence(order.itemsSubtotalInPence)}</span></div>
          <div className="flex justify-between"><span className="text-muted">Shipping</span><span>{formatPriceFromPence(order.shippingCostInPence)}</span></div>
          {order.vatAmountInPence > 0 && (
            <div className="flex justify-between"><span className="text-muted">VAT</span><span>{formatPriceFromPence(order.vatAmountInPence)}</span></div>
          )}
          <div className="flex justify-between pt-2 border-t border-border font-medium"><span>Total</span><span>{formatPriceFromPence(order.totalInPence)}</span></div>
        </div>
      </div>
      ...
      <div className="bg-compliance-amber-bg border border-compliance-amber-border p-5 mb-8">
        <p className="label-editorial text-compliance-amber-text mb-2">Research use reminder</p>
        <p className="text-sm text-compliance-amber-text leading-relaxed">
      ...
      {order.customer.uid === null && (
        <div className="bg-white border border-border p-6">
          <p className="font-serif text-xl text-navy mb-2">Want this to be easier next time?</p>
